...
void aliasAlreadyExist_lookupaddress(struct address_t* root,char* alias/*, struct address_t** temp*/){
 if(root!=NULL){
  aliasAlreadyExist_lookupaddress(root->left, alias);
   if(strcmp(root->alias,alias) == 0)
  ...
  aliasAlreadyExist_lookupaddress(root->right, alias);
 }
}
...
void lookUpAddress(struct address_t* root){
 char alias[20];
 struct address_t* flag = NULL;
 printf("\tEnter alias: ");
 scanf("%s",alias);
 struct address_t *ptr = root;

 aliasAlreadyExist_lookupaddress(ptr,alias);
 // if(flag != NULL)
// printf("\tAddress for the %s: %d.%d.%d.%d\n",flag->alias, flag->octet[0],flag->octet[1],flag->octet[2],flag->octet[3]);
