...
/*
int depth(struct address_t* root, struct address_t* node){
 if(node == NULL)
  return -1;

 int dist = -1;
 if((strcmp(root->alias, node->alias) == 0)
...
/*void utilDepth(struct address_t* root){
 if(root != NULL){
  utilDepth(root->left);
  root->depth = depth(root, root);
  utilDepth(root->right);
 }
}*/
...
void display(struct address_t *root){
 if(root!=NULL){
  display(root->left);
  printf("\t%s\t%d.%d.%d.%d\theight:%d\tdepth:%d\tparent: \n",root->alias, root->octet[0],root->octet[1],root->octet[2],root->octet[3],root->height,root->depth);
  display(root->right);
 }
}
