/*
 *HW3-G01413212.c -- Design and implement a C program based on the given specifications.
...
void utilDepth(struct address_t* root, int depth)
{
    ...
    {
        root->depth = depth;
        utilDepth(root->left, depth + 1); // left sub-tree
        utilDepth(root->right, depth + 1); // right sub-tree
    }
}
...
//adding a new address to the existing node
...
void addAddress(struct address_t* root){
 ...
 utilHeight(temp);//Calculating the height and depth after adding new node
 utilDepth(temp,0);
}
...
void display(struct address_t *root){
 if(root!=NULL){
  display(root->left);
  printf("\t%s\t%d.%d.%d.%d\theight:%d\tdepth:%d",root->alias, root->octet[0],root->octet[1],root->octet[2],root->octet[3],root->height,root->depth,root->parent->alias);
  if(root->parent != NULL)
   printf("\tparent: %s\n",root->parent->alias);
  else
   printf("\tparent: NONE\n");
